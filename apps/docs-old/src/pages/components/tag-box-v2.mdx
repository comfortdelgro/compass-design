...
import {BasicTagBox, WithErroredItemsTagBox} from '../../components/TagBoxV2'
...
<TagBoxV2
  ...
  onRemoveTag={(tagToBeRemoved) => console.log(tagToBeRemoved.id)}
  onEditTag={(tagToBeEdited) =>
    console.log(tagToBeEdited.id, tagToBeEdited.value)
  }
  css={{width: '70%'}}
/>
```
...
<TagBoxV2
  ...
  onRemoveTag={(tagToBeRemoved) => handleRemoveTag(tagToBeRemoved.id)}
  onEditTag={(tagToBeEdited) =>
    handleEditTag(tagToBeEdited.id, tagToBeEdited.value)
  }
  css={BasicTagBoxStyles}
/>
```

### With isErrored props for each tag

We added isErrored props in Tag object to help user define which items in Tags has errored

<section className={styles.avatarShowcaseContainer}>
  <WithErroredItemsTagBox />
</section>
