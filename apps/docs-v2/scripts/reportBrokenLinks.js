...
const {createRender} = require('@comfortdelgro/markdown')
...
const getMdFilesImported = (jsPageFile) => {
  ...
  return importPaths.map((importPath) => {
    let cleanImportPath = importPath.slice(
      1,
      importPath.length - "?@mui/markdown'".length,
    )
...
function save(lines) {
  ...
  fse.writeFileSync(
    path.join(docsSpaceRoot, '.link-check-errors.txt'),
    fileContents,
  )
}
...
if (require.main === module) {
  ...
  Object.keys(usedLinks)
    ...
    .filter((link) =>
      ['components-api', 'hooks-api', '#unstyled'].every(
        (str) => !link.includes(str),
      ),
    )
    .filter((link) =>
      UNSUPPORTED_PATHS.every(
        (unsupportedPath) => !link.includes(unsupportedPath),
      ),
    )
    .sort()
    .forEach((linkKey) => {
      ...
      console.log(`used in`)
      usedLinks[linkKey].forEach((f) =>
        console.log(`- ${path.relative(docsSpaceRoot, f)}`),
      )
      console.log('available anchors on the same page:')
      console.log(
        Object.keys(availableLinks)
          .filter(
            (link) => getPageUrlFromLink(link) === getPageUrlFromLink(linkKey),
          )
          .sort()
          .map(getAnchor)
          .join('\n'),
