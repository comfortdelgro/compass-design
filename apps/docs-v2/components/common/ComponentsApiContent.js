...
import Divider from 'components/common/ApiDivider'
import {ClassesTable, SlotsTable} from 'components/common/ApiPage'
                ...
                dangerouslySetInnerHTML={{
                  __html:
                    ...
                    classDescriptions[className].description
                      ...
                      .replace(
                        /{{nodeName}}/,
                        classDescriptions[className].nodeName,
                      ),
                }}
              />
            </tr>
...
export default function ComponentsApiContent(props) {
  ...
  React.useEffect(() => {
    const anchor =
      router.asPath.indexOf('#') >= 0 ? router.asPath.split('#')[1] : null
...
import { ${pageContent.name} } from '${source}';`}
          ...
          />
          <span
            dangerouslySetInnerHTML={{__html: t('api-docs.importDifference')}}
          />
          <Heading
            text='props'
          ...
          {cssComponent && (
            <React.Fragment>
              <span
                dangerouslySetInnerHTML={{
                  __html: t('api-docs.cssComponent').replace(
                    /{{name}}/,
                    pageContent.name,
                  ),
                }}
          ...
          {componentSlots?.length ? (
            <React.Fragment>
              <Heading
                text='slots'
                hash={`${componentNameKebabCase}-slots`}
                level='h3'
              />
              {slotGuideLink && (
                <p
