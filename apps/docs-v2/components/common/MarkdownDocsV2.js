...
import {getTranslatedHeader as getComponentTranslatedHeader} from 'components/common/ApiPage'
import {HEIGHT as AppFrameHeight} from 'components/common/AppFrame'
import AppLayoutDocs from 'components/common/AppLayoutDocs'
import {HEIGHT as TabsHeight} from 'components/common/ComponentPageTabs'
import ComponentsApiContent from 'components/common/ComponentsApiContent'
...
export default function MarkdownDocsV2(props) {
  ...
  Object.keys(componentsApiPageContents).forEach((key) => {
    ...
    const componentApiToc = [
      ...
      (classes?.classes?.length ||
        Object.keys(classes?.classes?.globalClasses || {}).length) &&
  ...
  while (i < rendered.length && !done) {
    ...
    if (
      ...
      renderedMarkdownOrDemo.component.indexOf('Tabs') >= 0
    ) {
      done = true
    }
    commonElements.push(
        ...
        <CssVarsProvider>
          ...
          {activeTab === '' &&
            rendered
              ...
              .slice(
                i,
                rendered.length - (headers.components.length > 0 ? 1 : 0),
              )
