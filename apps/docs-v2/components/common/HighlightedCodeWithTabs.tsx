...
const StyledTabPanel = styled(TabPanel)<{ownerState: {mounted: boolean}}>(
  ({ownerState}) => ({
    '& pre': {
      marginTop: 0,
...
const StyledTab = styled(Tab)<{ownerState: {mounted: boolean}}>(
  ({theme, ownerState}) =>
    theme.unstable_sx({
      py: 1.5,
      px: 2,
  ...
  React.useEffect(() => {
    try {
      setActiveTab((prev) => {
        ...
        return storedValues && availableTabs.includes(storedValues)
          ? storedValues
          : prev
      })
    } catch (error) {
