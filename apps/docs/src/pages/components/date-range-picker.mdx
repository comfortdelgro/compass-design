...
```js
import {DateRangePicker, RangeValue, CalendarProps, Calendar} from '@comfortdelgro/react-compass'

const {getLocalTimeZone, parseDate} =
  Calendar.InternationalizedDate
...
function ControlledDateRangePicker() {
  ...
  const onChangeDateRangePicker = (value: RangeValue<CalendarProps | null> | null) => {
...
```js
const {isWeekend} = Calendar.InternationalizedDate
...
<section className={styles.componentShowcaseContainer}>
  <DateRangePicker isMobile shouldCloseOnSelect />
</section>
