...
Provided appears to be a React component called DataGrid which is a table component built base on the @tanstack/react-table library.
...
The data for the data grid to display. This array should match the type you provided to `table.setRowType<...>`, but in theory could be an array of anything.
...
When the data option changes reference (compared via Object.is), the data grid will reprocess the data.
...
Make sure your data option is only changing when you want the data grid to reprocess.
Providing an inline [] or constructing the data array as a new object every time you want to render the data grid will result in a lot of unnecessary re-processing.
This can easily go unnoticed in smaller data grids, but you will likely notice it in larger data grids.
