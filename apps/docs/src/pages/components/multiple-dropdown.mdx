...
<MultipleDropdown
  ...
  placeholder='Choose an animal'
  values={value}
  onValuesChange={(k: 'all' | Set<Key>) => setValue(k === 'all' ? k : [...k])}
>
  <MultipleDropdown.Item value='panda'>
    <BugIcon /> Red Panda
  </MultipleDropdown.Item>
...
| values             | `Iterable<Key>`             | -       | The currently selected values in the collection (controlled).                                                                                             |
...
| defaultValues      | `Iterable<Key>`             | —       | The initial selected value in the collection (uncontrolled).                                                                                              |
| onLoadMore         | `() => void`                | —       | Handler when user scrolls to the end.                                                                                                                     |
| onValuesChange     | `(key: Key) => any`         | —       | Handler that is called when the selection changes.                                                                                                        |
