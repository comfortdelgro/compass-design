import ComponentDisplay from '../../components/ComponentDisplay'
import BugIcon from '@comfortdelgro/compass-icons/react/bug'
import {Radio} from '@comfortdelgro/react-compass'
import {Callout} from 'nextra-theme-docs'

# Radio

Radio allow users to select a single item from a list of mutually exclusive options.

<Callout>
  Check out our [ladle](/ladle?story=radio--variants) for more info on this
  component.
</Callout>

---

### Import

```js
import {Radio, RadioGroupProps, RadioProps} from '@comfortdelgro/react-compass'
```

or

```js
import {
  Radio,
  RadioGroupProps,
  RadioProps,
} from '@comfortdelgro/react-compass/radio'
```

We recommend the second method to import components for the benefits of [tree shaking](https://developer.mozilla.org/en-US/docs/Glossary/Tree_shaking).

---

### Basic Radio

<Radio.Group>
  <Radio value='1' />
  <Radio value='2' />
</Radio.Group>

```js
<Radio.Group>
  <Radio value='1' />
  <Radio value='2' />
</Radio.Group>
```

#### With label

<Radio.Group>
  <Radio value='1' label='male' />
  <Radio value='2' label='female' />
</Radio.Group>

```js
<Radio.Group>
  <Radio value='1' label='male' />
  <Radio value='2' label='female' />
</Radio.Group>
```

#### With label and description:

<Radio.Group>
  <Radio value='1' label='Option 1' description='Description of Option 1' />
  <Radio value='2' label='Option 2' description='Description of Option 2' />
</Radio.Group>

```js
<Radio.Group>
  <Radio value='1' label='Option 1' description='Description of Option 1' />
  <Radio value='2' label='Option 2' description='Description of Option 2' />
</Radio.Group>
```

#### Add tooltip:

Tooltips will display on the radio next to label as a icon

<Radio.Group>
  <Radio
    value='1'
    label='Option 1'
    description='Description of Option 1'
    tooltip='This is tooltip 1'
  />
  <Radio
    value='2'
    label='Option 2'
    description='Description of Option 2'
    tooltip='This is tooltip 2'
  />
  <Radio
    value='3'
    label='Option 3'
    description='Description of Option 3'
    tooltip='This is tooltip 3'
  />
  <Radio
    value='4'
    label='Option 4'
    description='Description of Option 4'
    tooltip='This is tooltip 4'
  />
</Radio.Group>

```js
<Radio.Group>
  <Radio
    value='1'
    label='Option 1'
    description='Description of Option 1'
    tooltip='This is tooltip 1'
  />
  <Radio
    value='2'
    label='Option 2'
    description='Description of Option 2'
    tooltip='This is tooltip 2'
  />
  <Radio
    value='3'
    label='Option 3'
    description='Description of Option 3'
    tooltip='This is tooltip 3'
  />
  <Radio
    value='4'
    label='Option 4'
    description='Description of Option 4'
    tooltip='This is tooltip 4'
  />
</Radio.Group>
```

#### Disabled radio button:

<Radio.Group>
  <Radio value='1' label='Option 1' />
  <Radio value='2' label='Option 2' isDisabled />
  <Radio value='3' label='Option 3' />
  <Radio value='4' label='Option 4' isDisabled />
</Radio.Group>

```js
<Radio.Group>
  <Radio value='1' label='Option 1' />
  <Radio value='2' label='Option 2' isDisabled />
  <Radio value='3' label='Option 3' />
  <Radio value='4' label='Option 4' isDisabled />
</Radio.Group>
```

### Variants

Outlined style

```js
<Radio.Group>
  <Radio
    variant='outlined'
    label={
      <>
        test <BugIcon />
      </>
    }
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='1'
    tooltip='This is tooltip'
  />
  <Radio
    variant='outlined'
    label='test2'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='2'
  />
  <Radio
    variant='outlined'
    label='test3'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='3'
    isDisabled
  />
</Radio.Group>
```

<ComponentDisplay>
  <Radio.Group>
    <Radio
      variant='outlined'
      label={
        <>
          test <BugIcon />
        </>
      }
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='1'
      tooltip='This is tooltip'
    />
    <Radio
      variant='outlined'
      label='test2'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='2'
    />
    <Radio
      variant='outlined'
      label='test3'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='3'
      isDisabled
    />
  </Radio.Group>
</ComponentDisplay>

Simple style as default

```js
<Radio.Group>
  <Radio
    variant='simple'
    label='test'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='1'
    tooltip='This is tooltip'
  />
  <Radio
    variant='simple'
    label='test2'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='2'
  />
  <Radio
    variant='simple'
    label='test3'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='3'
    isDisabled
  />
</Radio.Group>
```

<ComponentDisplay>
  <Radio.Group>
    <Radio
      variant='simple'
      label='test'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='1'
      tooltip='This is tooltip'
    />
    <Radio
      variant='simple'
      label='test2'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='2'
    />
    <Radio
      variant='simple'
      label='test3'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='3'
      isDisabled
    />
  </Radio.Group>
</ComponentDisplay>

### Orientation

Orientation horizontal

```js
<Radio.Group orientation='horizontal'>
  <Radio
    label='test'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='1'
    tooltip='This is tooltip'
  />
  <Radio
    label='test2'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='2'
  />
</Radio.Group>
```

<ComponentDisplay>
  <Radio.Group orientation='horizontal'>
    <Radio
      label='test'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='1'
      tooltip='This is tooltip'
    />
    <Radio
      label='test2'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='2'
    />
  </Radio.Group>
</ComponentDisplay>

Orientation vertical as default

```js
<Radio.Group orientation='vertical'>
  <Radio
    label='test'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='1'
    tooltip='This is tooltip'
  />
  <Radio
    label='test2'
    description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
    value='2'
  />
</Radio.Group>
```

<ComponentDisplay>
  <Radio.Group orientation='vertical'>
    <Radio
      label='test'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='1'
      tooltip='This is tooltip'
    />
    <Radio
      label='test2'
      description='Lorem ipsum dolor sit amet, consectetur adipiscing elit'
      value='2'
    />
  </Radio.Group>
</ComponentDisplay>
---

## Props

### Radio.Group Props:

| Name         | Type                             | Default  | Description                                                                             |
| :----------- | :------------------------------- | :------- | :-------------------------------------------------------------------------------------- |
| children\*   | [`ReactElement`, `ReactElement`] | —        | Childrent must be Radio component                                                       |
| orientation  | `vertical` \| `horizontal`       | vertical | The axis the Radio Button(s) should align with.                                         |
| name         | `string`                         | -        | The name of the RadioGroup, used when submitting an HTML form.                          |
| value        | `string`                         | -        | The current value (controlled).                                                         |
| defaultValue | `string`                         | -        | The current value (uncontrolled).                                                       |
| onChange     | `(value: boolean) => void`       | -        | Handler that is called when the value changes.                                          |
| onBlur       | `() => void`                     | -        | Handler that is called when click outside group.                                        |
| isDisabled   | `boolean`                        | false    | Whether the input is disabled.                                                          |
| isReadOnly   | `boolean`                        | false    | Whether the input can be selected but not changed by the user.                          |
| css          | `CSS`                            | —        | The system prop that allows defining system overrides as well as additional CSS styles. |

### Radio Props

| Name        | Type        | Default | Description                                                                                                            |
| :---------- | :---------- | :------ | :--------------------------------------------------------------------------------------------------------------------- |
| title       | `string`    | —       | Display title for radio                                                                                                |
| description | `string`    | —       | Display description for radio                                                                                          |
| rightLabel  | `ReactNode` | —       | Display rightLabel for radio                                                                                           |
| tooltip     | `string`    | —       | Display tooltip for radio                                                                                              |
| value       | `string`    | -       | The value of the radio button, used when submitting an HTML form.                                                      |
| isDisabled  | `boolean`   | false   | Whether the radio button is disabled or not. Shows that a selection exists, but is not available in that circumstance. |
| css         | `CSS`       | —       | The system prop that allows defining system overrides as well as additional CSS styles.                                |

---

<Callout>
  Check out our [ladle](/ladle?story=radio--variants) for more info on this
  component.
</Callout>
