...
export type HeaderColumnFilterProps<TData = unknown, TValue = unknown> = Props<TData, TValue> & 
  Omit<React.HTMLAttributes<HTMLDivElement>, keyof Props<TData, TValue>>;

const HeaderColumnFilter = forwardRef<HTMLInputElement, HeaderColumnFilterProps>(
  ({ column, table, css = {}, className }, ref) => {
    ...
    const handleToggleFiltering = (e: React.MouseEvent) => {
      e.stopPropagation();
      ...
      <CssInjection css={css} childrenRef={filterRef}>
        ...
        >
          <div className={rootClasses} onClick={(e) => e.stopPropagation()}>
            {typeof firstValue === 'number' ? renderNumberFilter() : renderTextFilter()}
          </div>
        </Popover>
      </CssInjection>
    );
