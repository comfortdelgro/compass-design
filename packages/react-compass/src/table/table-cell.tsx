...
export type TableCellProps<TData = unknown, TValue = unknown> = Props<TData, TValue> &
...
const TableCell = React.forwardRef<HTMLTableCellElement, TableCellProps>((props, ref) => {
  ...
  const { getValue, row: { index }, column: { id } } = cell;
  ...
  const renderGroupedCell = () => (
    <button
      ...
      style={{ cursor: row.getCanExpand() ? 'pointer' : 'normal', display: 'flex', alignItems: 'center' }}
    >
      {row.getIsExpanded() ? (
  ...
  const renderContent = () => {
    if (isCellEditable) {
      return <EditableCell cell={cell} getValue={getValue as never} row={index} column={id} />;
    }
