...
const Icon = () => (
  <svg className={styles.calendarToggleIcon} width='16' height='16' viewBox='0 0 16 16'>
    <path
...
const DateField = React.forwardRef<HTMLDivElement, Props>((props, ref) => {
  ...
  return (
    <div>
      <div
        className={`cdg-date-field ${styles.dateField} ${isDisabled ? styles.disabled : ''} ${isInvalid ? styles.invalid : ''}`}
      >
        {typeof props.label === 'string' ? (
          <span {...labelProps} className={`date-field-label ${styles.dateFieldLabel}`}>
      ...
      {props.isInvalid && props.errorMessage && (
        <div className={`cdg-date-field-error ${styles.textFieldHelperText} ${styles.helperTextError}`} >
          {props.errorMessage}
        </div>
      )}
      {props.helperText ? (
