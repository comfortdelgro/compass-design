...
const Select = React.forwardRef<HTMLDivElement, DropdownProps>((props, ref) => {
            ...
            case 'select':
                return (
                    ...
                    >
                        ...
                        >
                            {prefix}
                            <span className={`${styles.dropdownButtonText} cdg-dropdown-button-text`}>
            ...
            case 'combobox':
                return (
                    <div
                        ...
                        cdg-dropdown-input ${!selectedItem ? 'cdg-dropdown-item-right-icon-isEmpty' : ''}
                        ${isDisabled ? 'cdg-dropdown-item-right-icon-isDisabled' : ''}
                        ${isErrored ? 'cdg-dropdown-item-right-icon-isErrored' : ''}
                        `}
                        role='button'
                        aria-required={isRequired}
                    >
    ...
    return (
        <CssInjection css={css} childrenRef={selectRef}>
            <div
                className={`${styles.dropdownWrapper} ${open ? styles.dropdownOpening : ''} ${open ? styles.dropdownH5 : ''}`}
                ref={selectRef}
                onKeyDown={handleKeyDown}
                {...htmlProps}
            >
                {label && (
                    <label htmlFor={id} id={`${id}-label`} className={`${styles.dropdownLabel} cdg-dropdown-label`}>
                        {label}
                ...
                {isErrored && errorMessage && (
                    <div className={`${styles.dropdownHelperText} ${isErrored ? 'cdg-dropdown-helper-isErrored' : ''}`}>{errorMessage}</div>
                )}
                {helperText && <div className={`${styles.dropdownHelperText}`}>{helperText}</div>}
            </div>
        </CssInjection>
    )
})
