...
const CarouselSlider = React.forwardRef<HTMLDivElement, CarouselSliderProps>((props, ref) => {
  ...
  const handlePointerDown = (event: React.PointerEvent) => {
    ...
    if (scroller.current) {
      ...
      const handlePointerUp = (event: PointerEvent) => {
        ...
        if (scroller.current) {
          scroller.current.removeEventListener('pointermove', handlePointerMove)
  ...
  const toEffectClassName = () => {
    return 'effect' + effect.charAt(0).toUpperCase() + effect.slice(1)
  }

  const toNextButtonTextClassName = () => {
    return (
      'sliderNextButton' +
      navigationButtonType.charAt(0).toUpperCase() +
      navigationButtonType.slice(1)
    )
  }

  const toPrevButtonTextClassName = () => {
    return (
      'sliderPrevButton' +
      navigationButtonType.charAt(0).toUpperCase() +
      navigationButtonType.slice(1)
  ...
  return (
    <CssInjection css={css} childrenRef={hostRef}>
      <div
        className={`content-slider ${styles[toEffectClassName()]} ${
          styles.carousel
        } ${className ? ' ' + className : ''}`}
        ref={sliderRef}
        style={style}
          ...
          >
            <div
              ...
              className={`${styles.sliderNavButton} ${
                ...
                styles[toPrevButtonTextClassName()]
              }`}
            >
              {navigationButtonType === 'icon' ? (
            ...
            <div
              ...
              className={`${styles.sliderNavButton} ${
                ...
                styles[toNextButtonTextClassName()]
              }`}
            >
              {navigationButtonType === 'icon' ? (
                <svg viewBox='0 0 320 512'>
